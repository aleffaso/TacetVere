...
slideReset = (direction) => {
  if (direction === "custom") {
    translate += 1200 - translateAmount;
    document.querySelector("#atmmos-carbon-management-info").reset();
  } else if (direction === "main") {
    translate += 200 - translateAmount;
    document.querySelector("#atmmos-carbon-management").reset();
  }
  pages.forEach(
    (pages) => (pages.style.transform = `translateX(${translate}%)`)
  );
...
slideThree = (direction) => {
  direction === "next"
    ...
    : (translate += 1300 - translateAmount);
  pages.forEach(
    (pages) => (pages.style.transform = `translateX(${translate}%)`)
  );
};
